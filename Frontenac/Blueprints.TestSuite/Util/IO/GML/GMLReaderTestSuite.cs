    ...
    public abstract class GmlReaderTestSuite : TestSuite
    {
        ...
        public void TestReadingTinkerGraphExample3()
        {
            ...
            try
            {
                if (graph.Features.IgnoresSuppliedIds || !graph.Features.SupportsEdgeIteration ||
                    !graph.Features.SupportsVertexIteration) return;

                StopWatch();

                using (var stream = GetResource<GmlReaderTestSuite>("graph-example-3.gml"))
                {
                    var gmlReader = new GmlReader(graph) {EdgeIdKey = GmlTokens.Id};
        ...
        public void TestReadingTinkerGraphExample3MappingLabels()
        {
            ...
            try
            {
                ...
                using (var stream = GetResource<GmlReaderTestSuite>("graph-example-3.gml"))
                {
                    var gmlReader = new GmlReader(graph) {EdgeLabelKey = "label2", EdgeIdKey = "id"};
                    gmlReader.InputGraph(stream, 1000);
                }
        ...
        public void TestReadingTinkerGraphExample3MappingIDs()
        {
            ...
            try
            {
                ...
                using (var stream = GetResource<GmlReaderTestSuite>("graph-example-3.gml"))
                {
                    var gmlReader = new GmlReader(graph) {VertexIdKey = "id2", EdgeIdKey = "id2"};
                    gmlReader.InputGraph(stream, 1000);
                }
        ...
        public void TestMigratingTinkerGraphExample3()
        {
            ...
            try
            {
                ...
                using (var stream = GetResource<GmlReaderTestSuite>("graph-example-3.gml"))
                {
                    var gmlReader = new GmlReader(graph) {EdgeIdKey = "id"};
                    gmlReader.InputGraph(stream, 1000);
                }
                ...
                StopWatch();
                // FIXME Should not explicitly define the Graph type (TinkerGrapĥ)
                // here. Need to accept 2 graphs as input params?
                var toGraph = new TinkerGrapĥ();
