        ...
        public void TestReadingTinkerGraph()
        {
            ...
            try
            {
                if (!graph.Features.IgnoresSuppliedIds)
                {
                    ...
                    using (
                        var stream =
                            typeof (GraphMlReaderTestSuite).Assembly.GetManifestResourceStream(
                                typeof (GraphMlReaderTestSuite), "graph-example-1.xml"))
                    {
            ...
            try
            {
                if (!graph.Features.IgnoresSuppliedIds)
                {
                    ...
                    using (
                        var stream =
                            typeof (GraphMlReaderTestSuite).Assembly.GetManifestResourceStream(
                                typeof (GraphMlReaderTestSuite), "graph-example-4.xml"))
                    {
                        new GraphMlReader(graph).InputGraph(stream);
                    }
