    ...
    public abstract class IndexTestSuite : TestSuite
    {
        ...
        public void TestPutGetRemoveVertex()
        {
            ...
            if (graph.GetFeatures().SupportsVertexIndex)
            {
                ...
                PrintPerformance(graph.ToString(), 1, "vertex removed and automatically removed from index", StopWatch());
        ...
        public void TestPutGetRemoveEdge()
        {
            ...
            if (graph.GetFeatures().SupportsEdgeIndex)
            {
                ...
                StopWatch();
                graph.RemoveEdge(e1);
                PrintPerformance(graph.ToString(), 1, "edge removed and automatically removed from index", StopWatch());
                Assert.AreEqual(Count(index.Get("dog", "puppy")), 0);
                Assert.AreEqual(e2, index.Get("dog", "mama").First());
